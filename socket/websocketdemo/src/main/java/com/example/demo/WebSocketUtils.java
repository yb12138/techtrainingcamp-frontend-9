...
public class WebSocketUtils {
    ...
    public static void startGame(Long roomID, String username) {
        ...
        List<PlayRoom> playRoomList =
                rooms.stream().filter(item -> item.getRoomId() == roomID).collect(Collectors.toList());
        if (playRoomList.size() == 1) {
            PlayRoom playRoom = playRoomList.get(0);
            List<Player> players = playRoom.getPlayerList();
            int old_count = playRoom.getPreNumber();
            int player_size = players.size();
            int actual_number = playRoom.getActualPlayerNumber();
            int max_number = playRoom.getMaxPlayerNumber();
            if (old_count + 1 == player_size) {
                // 房间内所有人都已经准备就绪，开始游戏
            ...
            } else if (old_count + 1 < player_size) {
                playRoom.setPreNumber(old_count + 1);
                for (Player temp : players) {
            ...
            // 判断并设置房间实际在线人数
            if (actual_number + 1 <= max_number) {
                playRoom.setActualPlayerNumber(actual_number + 1);
            }
            sendAllRoom();
        }
    }
